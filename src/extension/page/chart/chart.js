...
function drawChart(timerData) {
  ...
  dataArray.forEach(item => {
    ...
    if (item.isEmpty) {
      ...
      // Calculate the width percentage of the bar based on maxTotal
      const totalBarWidthPercent = (item.total / maxTotal) * 85; // 80% of bar-container
      const focusBarWidthPercent = (item.focus / maxTotal) * 85; // 80% of bar-container

      const totalBar = document.createElement('div');
      totalBar.className = 'bar bar-total';
      totalBar.style.width = `${totalBarWidthPercent}%`;

      const focusBar = document.createElement('div');
      focusBar.className = 'bar bar-focus';
      focusBar.style.width = `${focusBarWidthPercent}%`;
