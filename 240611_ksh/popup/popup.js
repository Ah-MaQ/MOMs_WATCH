...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (toggleBody) {
      chrome.storage.local.get(["switch", "firstToggle", "webcamTabId"], result => {
          if (result.switch) {
          ...
          if (result.switch && !result.firstToggle) {
            setTimeout(() => {
                chrome.tabs.create({ url: chrome.runtime.getURL('../cam/webcam.html') }, tab => {
                    webcamTabId = tab.id;
                    chrome.storage.local.set({ firstToggle: true, webcamTabId: tab.id });
                });
            }, 200); // 200ms 지연
        }
    });
    ...
    toggleBody.addEventListener("click", () => {
        ...
        if (isOn) {
          ...
          // 토글이 꺼질 때 firstToggle 상태를 초기화하고, 웹캠 탭을 닫습니다.
          chrome.storage.local.set({ firstToggle: false });
          chrome.storage.local.get("webcamTabId", result => {
              if (result.webcamTabId) {
                  chrome.tabs.remove(result.webcamTabId);
              }
          });
      }
    });
