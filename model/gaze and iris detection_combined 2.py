...
def draw_gaze(a, b, c, d, image_in, gaze_angles, mh=900, mw=1600):
    ...
    # # 시선 끝점 계산
    # gaze_end = (int(pos[0] + dx), int(pos[1] + dy))
    #
    # # 시선 화살표 그리기
    # cv2.arrowedLine(output_img, pos, gaze_end, (0, 0, 255), 2, tipLength=0.3)
...
# 관심 영역 설정 (화면의 가장자리 1cm 마진 제외, 아래쪽 제외)
def define_aoi(monitor_height, monitor_width, monitor_horizontal_res, monitor_vertical_res, margin_cm=1):
...
if __name__ == "__main__":
    # 사용자에게 모니터 크기 입력 받기
    monitor_height = float(input("모니터 높이 (inch): "))
    monitor_width = float(input("모니터 너비 (inch): "))
    monitor_horizontal_res = int(input("모니터 가로 해상도 (pixel): "))
    monitor_vertical_res = int(input("모니터 세로 해상도 (pixel): "))
    ...
    while True:
        ...
        if aoi is None:
            aoi, dpi = define_aoi(monitor_height, monitor_width, monitor_horizontal_res, monitor_vertical_res)
